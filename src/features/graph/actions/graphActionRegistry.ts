...
export const GRAPH_ACTION_IDS = [
  ...
  'graph.node.create',
  'graph.node.update',
  'graph.node.assign_agent',
  'graph.node.update_agent_status',
  'graph.node.clear_agent',
  'graph.node.delete',
  'graph.edge.create',
  'graph.edge.delete',
  'graph.user.add',
    ...
    payload: {
      ...
      fields: [
        { name: 'cardId', type: 'string', required: true, description: 'Target node id.' },
        {
          ...
          type: 'enum',
          required: true,
          description: 'Worker mode for the card.',
          values: ['human', 'agent'],
        },
        {
          name: 'agentConfig',
          type: 'object',
          required: true,
          description: 'Agent assignment details (type/command/model).',
        },
      ],
    },
    preconditions: ['Target card must exist.', 'When workerType is agent, agentConfig.type must be provided.'],
    undoable: true,
    apiCandidate: true,
    triggers: ['Card detail: Worker set to Agent', 'Card detail: Agent config edits'],
    handlerRefs: [
      'src/features/graph/components/CardDetailPanel.tsx#handleWorkerTypeChange',
      'src/features/graph/components/CardDetailPanel.tsx#commitAgentConfig',
      'src/features/graph/DagbanGraph.tsx#handleAssignAgent',
      'src/components/ProjectView.tsx#handleAssignAgent',
      'src/app/page.tsx#handleAssignAgent',
    ],
  },
  {
    id: 'graph.node.update_agent_status',
    ...
    payload: {
      ...
      fields: [
        { name: 'cardId', type: 'string', required: true, description: 'Target node id.' },
        {
          ...
          type: 'enum',
          required: true,
          description: 'Latest status reported for the agent.',
          values: ['idle', 'running', 'awaiting-review', 'approved', 'rejected'],
        },
        {
          name: 'agentBranch',
          type: 'string',
          required: false,
          description: 'Optional branch associated with current run/review.',
        },
        {
          name: 'agentSessionId',
          type: 'string',
          required: false,
          description: 'Optional resumable session identifier.',
        },
      ],
    },
    preconditions: ['Target card must exist.', 'Card should already be assigned to an agent.'],
    undoable: true,
    apiCandidate: true,
    triggers: ['Bridge lifecycle status updates', 'Manual review transitions'],
    handlerRefs: [
      'src/hooks/useBridgeConnection.ts#onAgentStatus',
      'src/features/graph/DagbanGraph.tsx#handleBridgeAgentStatus',
      'src/components/ProjectView.tsx#handleUpdateAgentStatus',
      'src/app/page.tsx#handleUpdateAgentStatus',
    ],
  },
  {
    id: 'graph.node.clear_agent',
    label: 'Clear Agent',
    kind: 'domain',
    domain: 'node',
    payload: {
    ...
    undoable: true,
    apiCandidate: true,
    triggers: ['Card detail: Worker set to Human'],
    handlerRefs: [
      'src/features/graph/components/CardDetailPanel.tsx#handleWorkerTypeChange',
      'src/features/graph/DagbanGraph.tsx#handleClearAgent',
      'src/components/ProjectView.tsx#handleClearAgent',
      'src/app/page.tsx#handleClearAgent',
    ],
  },
  {
